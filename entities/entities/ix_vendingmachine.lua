...
ENT.MaxRenderDistance = math.pow(256, 2.5)
--ENT.MaxStock = 4
ENT.Items = {
	{"WATER", "water", 10, 8, "drink"},
	{"SPARKLING", "water_sparkling", 12, 10, "drink"},
	{"SODA", "soda", 15, 10, "drink"},
	{"SPECIAL", "water_special", 20, 5, "drink"},
	{"CHIPS", "chips", 7, 10, "snack"},
	{"SARDINES", "sardines", 12, 10, "snack"}
}
...
if (SERVER) then
	...
	function ENT:Use(client)
		...
		if client:IsCombine() then canRestock = true end
		if client:IsCWU() then canRestock = true end
		...
		if (!canRestock or !client:KeyDown(IN_WALK)) then
			...
			if (self:GetStock(buttonID) > 0) then
				local forward = 20
				if buttonID == 6 or buttonID == 5 then 
					forward = 26
				end
