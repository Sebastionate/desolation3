...
function SWEP:OnRaised()
	...
	--self.Owner:Fire("activatebaton", 1)
end
...
function SWEP:PrimaryAttack()
	...
	if (self.Owner:KeyDown(IN_WALK)) then
		if (SERVER) then
			...
			if (ix.anim.GetModelClass(model) == "metrocop"
			or ix.anim.GetModelClass(model) == "mpfpack" ) then
