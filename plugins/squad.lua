...
local function CanUseSquadMenu(ply)
	if not ply then return false end
	if ply:IsCombine() then return true end
	if ply:Team() == FACTION_DISPATCH then return true end
...
ix.command.Add("squadmenu", {
	...
	OnRun = function(self, client)
		if CanUseSquadMenu(client) then
...
if CLIENT then
	...
	hook.Add("PreDrawHalos", "ixDrawSquadHalos", function()
		...
		if ix.option.Get("SquadHalos") then
			if squad then
				for k, v in pairs(squad) do
					if v:GetNetVar("squadleader") then
						halo.Add({v}, Color(100, 255, 100, 50), 2, 2, 2, true, true)
						table.RemoveByValue(squad, v)
						break
					end
				end

				halo.Add(squad, Color(255, 255, 255, 50), 2, 2, 2, true, true)
			end
		end
	end)
	...
	netstream.Hook("CreateSquad", function(ply, data)
		if !CanUseSquadMenu(ply) then ix.plugin.Get("adminutil"):WarnExploits(ply, "CreateSquad") return end
	...
	netstream.Hook("InviteToSquad", function(ply, data)
		if !CanUseSquadMenu(ply) then ix.plugin.Get("adminutil"):WarnExploits(ply, "InviteToSquad") return end
		if !IsValid(data[1]) then return end
		if !data[1]:IsCombine() then return end
	...
	netstream.Hook("AcceptInvite", function(ply, data)
		if !CanUseSquadMenu(ply) then ix.plugin.Get("admintuil"):WarnExploits(ply, "AcceptInvite") return end
