...
function Schema:AddCombineDisplayMessage(text, color, exclude, ...)
	...
	for _, v in ipairs(player.GetAll()) do
		if ((v:IsCombine() or v:Team() == FACTION_DISPATCH) and v != exclude) then
...
function Schema:SaveTerminals()
	...
	for _, v in ipairs(ents.FindByClass("civil_terminal")) do
		data[#data + 1] = {v:GetPos(), v:GetAngles(), v:GetTerminalID()}
	end

	ix.data.Set("civilTerminals", data)
end
...
function Schema:SaveMPFRationDispensers()
	local data = {}

	for _, v in ipairs(ents.FindByClass("ix_mpfrationdispenser")) do
		data[#data + 1] = {v:GetPos(), v:GetAngles(), v:GetEnabled()}
	end

	ix.data.Set("rationDispensersMPF", data)
end
	...
	for _, v in ipairs(ents.FindByClass("ix_combinelock")) do
		if (IsValid(v.door)) then
			data[#data + 1] = {
				...
				v:GetCWUAccess(),
				v:GetLoyalistAccess(),
				v.door:GetNWBool("CombineLocked", true)
			}
		end
	end
...
function Schema:LoadTerminals()
	for _, v in ipairs(ix.data.Get("civilTerminals") or {}) do
		local terminal = ents.Create("civil_terminal")

		terminal:SetPos(v[1])
		terminal:SetAngles(v[2])
		terminal:Spawn()
		terminal:SetTerminalID(v[3])
		local physObj = terminal:GetPhysicsObject()
...
function Schema:LoadMPFRationDispensers()
	for _, v in ipairs(ix.data.Get("rationDispensersMPF") or {}) do
...
function Schema:LoadCombineLocks()
	for _, v in ipairs(ix.data.Get("combineLocks") or {}) do
		...
		if (IsValid(door) and door:IsDoor()) then
			...
			lock:SetLocked(v[4])
			lock:SetCWUAccess(v[5])
			lock:SetLoyalistAccess(v[6])
...
function Schema:CreateScanner(client, class)
	...
	timer.Create(uniqueID, 0.33, 0, function()
		...
		if client:GetObserverTarget() ~= entity then
			entity:Remove()
